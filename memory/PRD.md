...
- ✅ **Faz 4 — Oyunlaştırma Motoru** (2026-08-21): **Kudos (7.12)** — çalışanlar arası takdir; önceden tanımlı 6 değer (Takım Oyunu/İnovasyon/Müşteri Odaklılık/Liderlik/Güvenilirlik/Pozitif Enerji) renkli chip; opsiyonel merkezi moderasyon (admin switch, varsayılan KAPALI → anında yayın); Akış/Aldıklarım/Verdiklerim sekmeleri; puan bağlantısı. **Rozet / Oyunlaştırma (7.13)** — merkezi puan ekonomisi (points_ledger): kudos alma +10, kudos verme +2, doğru cevap +5, tam puan bonusu +15 (admin'den düzenlenebilir); 7 seviye (Çaylak→Efsane) ilerleme çubuğuyla; 10 rozet (kazanılan/kilitli); streak (üst üste aktif gün); şirket liderlik tablosu. **Oyun (7.17)** — quiz formatı (soru havuzu, çoktan seçmeli, soru başına süreli geri sayım); tek seferlik oynama + otomatik puanlama; oyun bazında skor tablosu; is_tournament bayrağı (otomatik periyodik turnuva — gerçek cron MOCK). Koleksiyonlar: gami_config, points_ledger, kudos, games, game_plays. Endpoint'ler: /gami/config|leaderboard|profile, /kudos/*, /games/*. Testing (iteration_17): backend 12/12, frontend %100. **Faz 4 tamamlandı.**

- ✅ **Faz 3b — Faz 3 tamamlandı** (2026-08-21): **Şirketin Enleri (7.16)** — ödül başlıkları (yöntem: otomatik/manuel/oylama/karma + periyot), manuel kazanan belirleme, çalışan kazanan galerisi + oylama. **Kutlama (7.18)** — mock doğum/işe giriş tarihleri, alt tip başına görsel şablon galerisi, İK verisinden otomatik (bu ay doğum günü/kıdem/yeni başlayan) kutlama kartları. **Etkinlik geliştirmeleri (7.9)** — kontenjan (dolunca RSVP 400 'Kontenjan dolu'), servis bağlantısı ("Servis kullanacağım" + güzergah seçimi), tek QR + "Alandayım" check-in, raporda RSVP vs fiili katılım (QR) vs servis kullanımı. Koleksiyonlar: awards/award_winners/award_votes, cel_templates, event_checkins. Testing (iteration_16): backend 8/8, frontend %100. **Faz 3 (7.9/7.11/7.15/7.16/7.18) tamamlandı.**
